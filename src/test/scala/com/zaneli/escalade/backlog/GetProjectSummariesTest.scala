...
class GetProjectSummariesTest extends Specification with Mockito with TestUtil {

  "全ての参加プロジェクト状況を取得" should {

    "指定パラメータ無し" in {
      using(new ByteArrayOutputStream()) { request =>
        val client = getClient("GetProjectSummaries.xml", request)
        val result = client.getProjectSummaries
        getActualRequest(request) must_== getExpectedRequest("GetProjectSummaries.xml")
  ...
  def assertResponse(projectSummaries: Array[ProjectSummary]) = {
    projectSummaries.size must_== 1;
    {
      val projectSummary = projectSummaries(0)
